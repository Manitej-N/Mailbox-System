    ...
    while command != 'end':
        ...
        match key[0]:
            ...
            case 'add':
                ...
                if "%%" not in line:
                    print("Error: Missing %% before message body.")
                    break

                before, info = line.split('%%',1)
                #break first section
                ...
                mba.add_email(frm,to,date,subject,tag,info)
            ...
            case 'fnd':
                ...
                if len(key) < 2:
                    print("Usage: fnd <dd/mm/yyyy>")
                    continue
                date_fnd = key[1]
                received = mba.find(date_fnd)
                if received:
                    print(f"No emails found on {date_fnd}")
            ...
            case 'get' :                # retrieve and display email Mail object given email ID
                ...
                if len(key) < 2:
                    ...
                    continue
                choose = key[1]
                found = mba.get_email(choose)
                if found:
                    found.show_email()
            case 'lst' :                # display entire mailbox
                # example command prompt:
                # lst
                for mails in mba._mailbox:
                    mails.show_email()
            case 'mrkr':
                # example command prompt:
                # mrkr 10
